...
class GameBoard {
  ...
  init() {
    ...
    this.points = 0
    this.clearLines = 0
    this.board = Array.from(new Array(this.height), () => new Array(this.width).fill(0))
    this.shape = ShapeFactory.newShape()
    this.initShapePosition()
    this.nextShape = ShapeFactory.newShape()
    setTimeout(() => this.observer.onNextShapeChanged(this.nextShape.coordinates(), this.nextShape.colorCode))
  ...
  initShapePosition() {
    ...
    const pos = this.shape.coordinates().map((v) => [this.x + v[0], this.y + v[1]])
  ...
  moveLeft() {
    ...
    const from = this.shape.coordinates().map((v) => [this.x + v[0], this.y + v[1]])
    const to = from.map((v) => [v[0] - 1, v[1]])
    this.x -= 1
    setTimeout(() => this.observer.onShapeMoved(from, to, this.shape.colorCode))
  }
  ...
  rotate() {
    ...
    const to = this.shape.coordinates().map((v) => [this.x + v[0], this.y + v[1]])
    //console.log(to)
    setTimeout(() => this.observer.onShapeMoved(from, to, this.shape.colorCode))
  }

  solidShape() {
    this.shape.coordinates().forEach((v) => this.board[this.y + v[1]][this.x + v[0]] = this.shape.colorCode)
  }
